...
class LocalArticleSource {
  ...
  /// Returns the series_id for the given article URL + title, creating the
  /// website and/or series record if they don't exist yet.
  /// If [seriesName] is provided (e.g., from novelarrow.com API), it's used as the
  /// series name instead of extracting from article title.
  Future<String> _resolveOrCreateSeriesId(
    Database db,
    String url,
    String articleTitle, {
    String? seriesName,
    ...
    // Use seriesName if provided (e.g., from API), otherwise extract from article title
    final sName = seriesName ?? TitleExtractor.extractBookTitle(articleTitle);
    ...
    // Find or create series row.
    final sRows = await db.query(
      'series',
      columns: ['id'],
      where: 'name = ? AND website_id = ?',
      whereArgs: [sName, websiteId],
      limit: 1,
    );
    if (sRows.isNotEmpty) {
      return sRows.first['id'] as String;
    }
  ...
  Future<void> insertOrUpdate(Article article) async {
    final db = await _db;
    final seriesId = await _resolveOrCreateSeriesId(
      db,
      article.url,
      article.title,
